<script lang="ts">
	...
	const techStack = [
		'SvelteKit', 'TypeScript', 'Node.js', 'PostgreSQL',
		'Docker', 'TailwindCSS', 'NestJS', 'Next.js'
	];
</script>
	...
	<div class="container mx-auto max-w-6xl px-4 py-24 md:py-32">
		<div class="max-w-2xl">
			...
			<h1 class="text-4xl font-bold tracking-tight sm:text-5xl md:text-6xl mb-4">
				Hi, I'm <span class="text-primary">{data.profile?.full_name ?? 'Bagus'}</span>
			</h1>
			...
			<div class="flex items-center gap-3">
				{#if data.profile?.github_url}
					...
					<a href={data.profile.github_url}
					target="_blank"
				...
				{#if data.profile?.linkedin_url}
					...
					<a href={data.profile.linkedin_url}
					target="_blank"
					rel="noopener noreferrer"
					class="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
					aria-label="LinkedIn"
...
{#if data.featuredProjects.length > 0}
	<section class="container mx-auto max-w-6xl px-4 py-20">
		<div class="flex items-end justify-between mb-10">
			...
			<a href="/portfolio" class="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors">
